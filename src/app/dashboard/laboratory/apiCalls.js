...
//     `${process.env.NEXT_PUBLIC_BASE_URL}/api/laboratory/draft-template`
//   );
...
export async function getDraftDocument() {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_BASE_URL}/api/laboratory/draft-document`
	);
...
export async function getLaboratoryDocuments(searchQuery) {
	let baseUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/api/laboratory/get-laboratory-documents`;
...
export async function getDocumentById(_id) {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_BASE_URL}/api/laboratory/document-by-id/${_id}`
	);
...
export async function getLaboratoryDocumentNumber(searchQuery) {
	let baseUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/api/laboratory/get-laboratory-document-number`;
...
export async function getCustomers() {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_BASE_URL}/api/customers/all`
	);
